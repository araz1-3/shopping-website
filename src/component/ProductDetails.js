                    ...
                    <div className="flex justify-between gap-5 flex-wrap flex-row xs:flex-col items-center">
                        <div className="flex items-start pb-1 flex-row xs:flex-col justify-center">
                            ...
                            <div className="mt-[1px]">
                                {quantityCount(state,product.id) > 1 && <button className="w-[30px] pl-[5px]  hover:bg-[#0b499b] text-[1.6rem] font-bold leading-[30px] transition-all duration-100 ease-in h-[30px] bg-[#1a73e8]  text-white rounded-[4px] cursor-pointer ml-[5px] shadow-newShadow" onClick={()=>dispatch({type:"DECREASE",payload:product})}>
                                    <svg xmlns="http://www.w3.org/2000/svg" className=" h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                        <path fillRule="evenodd" d="M5 10a1 1 0 011-1h8a1 1 0 110 2H6a1 1 0 01-1-1z" clipRule="evenodd" />
                                    </svg>
                                     </button>}
                                {quantityCount(state,product.id) === 1 && <button className="w-[30px] pl-[5px] hover:bg-red-700 text-[1.6rem] font-bold leading-[30px] transition-all duration-100 ease-in h-[30px] bg-red-600  text-white rounded-[4px] cursor-pointer ml-[5px] shadow-newShadow"   onClick={()=>dispatch({type:"REMOVE_ITEM",payload:product})}><img alt="trash" src={Trash} className="w-[20px] text-center"/></button>}
                                {quantityCount(state,product.id) > 0 && <span className="inline-block pt-2 pl-1 w-[20px] font-bold ml-[5px] text-[1.2rem] text-[#1a73e8]">{quantityCount(state,product.id)}</span>}
                                {
                                    isInCart(state,product.id)
                                        ? <button className="w-[30px] text-[1.6rem] hover:bg-[#0b499b] font-bold pl-[5px]  leading-[30px] transition-all duration-100 ease-in h-[30px] bg-[#1a73e8]  text-white rounded-[4px] cursor-pointer ml-[5px] shadow-newShadow"  onClick={()=>dispatch({type:"INCREASE",payload:product})}>
                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
