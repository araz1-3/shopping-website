    ...
    return (
        <div className="border relative border-gray-200 hover:shadow-2xl border-solid rounded bg-white w-[250px] transition-all ease-in overflow-hidden mt-0 mb-[50px] mx-[10px] text-center">
            ...
            <div className="flex justify-between items-center mt-[35px] mb-[20px] mx-[20px] h-[40px]">
                ...
                <div>
                    {quantityCount(state,productData.id) > 1 && <button className="w-[30px] hover:bg-[#0b499b] text-[1.6rem] font-bold leading-[30px] transition-all duration-100 ease-in h-[30px] bg-[#1a73e8]  text-white rounded-[4px] cursor-pointer ml-[5px] shadow-newShadow" onClick={()=>dispatch({type:"DECREASE",payload:productData})}>-</button>}
                    {quantityCount(state,productData.id) === 1 && <button className="w-[30px] pl-1 hover:bg-red-700 text-[1.6rem] font-bold leading-[30px] transition-all duration-100 ease-in h-[30px] bg-red-600  text-white rounded-[4px] cursor-pointer ml-[5px] shadow-newShadow"   onClick={()=>dispatch({type:"REMOVE_ITEM",payload:productData})}><img alt="trash" src={Trash} className="w-[20px] text-center"/></button>}
                    {quantityCount(state,productData.id) > 0 && <span className="inline-block pt-2 w-[20px] font-bold ml-[5px] text-[1.2rem] text-[#1a73e8]">{quantityCount(state,productData.id)}</span>}
                    {
                        isInCart(state,productData.id)
                            ? <button className="w-[30px] text-[1.6rem] hover:bg-[#0b499b] font-bold leading-[30px] transition-all duration-100 ease-in h-[30px] bg-[#1a73e8]  text-white rounded-[4px] cursor-pointer ml-[5px] shadow-newShadow"  onClick={()=>dispatch({type:"INCREASE",payload:productData})}>+</button>
